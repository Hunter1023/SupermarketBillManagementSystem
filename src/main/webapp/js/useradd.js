...
$(function () {
    ...
    reEnterUserPwd.bind("focus", function () {
        validateTip(reEnterUserPwd.next(), {"color": "#666666"}, "* 请输入与上面一致的密码", false);
    }).bind("blur", function () {
        if (reEnterUserPwd.val() != null && reEnterUserPwd.val().length >= 6
            && reEnterUserPwd.val().length < 20 && userPassword.val() == reEnterUserPwd.val()) {
            validateTip(reEnterUserPwd.next(), {"color": "green"}, imgYes, true);
        } else {
            validateTip(reEnterUserPwd.next(), {"color": "red"}, imgNo + " 两次密码输入不一致，请重新输入", false);
        }
    });
    ...
    addBtn.bind("click", function () {
        if (userCode.attr("validateStatus") != "true") {
            ...
            userPassword.blur();
        } else if (reEnterUserPwd.attr("validateStatus") != "true") {
            reEnterUserPwd.blur();
